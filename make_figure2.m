...
if iP == 1
    ...
    plot(ax(1,iP),[-100,x(1)],[1,1], 'linewidth', 1.5,  'Color',alexred)
    plot(ax(1,iP),x,H-H1,'linewidth', 1.5,  'Color',alexred);
    plot(ax(1,iP),[-100,x(1)],[0,0], 'linewidth', 1.5 , 'Color',alexred)
end
%drawnow; pause
...
ax(2,iP).YLabel.String = '$\bar{T}$';
ax(2,iP).XTickLabel = {};
txt{iP} = sprintf("$T_0 - T_f = %.1f $", T2 - T1);
...
%% add the text title
t1 = cell2mat(txt{1,1});
txt1 = text(ax(1,1), -9, 2.3, t1, 'FontSize', 18);
t2 = cell2mat(txt{1,2});
txt2 = text(ax(1,2), -3.4, 2.3, t2, 'FontSize', 18);
